...
def render_glb_to_image(
    glb_path, output_image_path, width=800, height=600, dpi=100
):
    ...
    try:
        ...
        if mesh is None or not isinstance(mesh, trimesh.Trimesh):
            ...
            img = Image.new(
                "RGB", (width, height), color=(100, 149, 237)
            )  # Cornflower blue
        ...
        max_range = max(
            [
                ...
                vertices[:, 2].max() - vertices[:, 2].min() * 2,
            ]
        )  # Amplify z range
        ...
        img = Image.new(
            "RGB", (width, height), color=(255, 0, 0)
        )  # Red for error
        img.save(output_image_path)
...
def create_ocean_animation(
    num_frames=100,
    time_step=0.1,
    output_gif="ocean_animation.gif",
    duration=0.1,
    width=20,
    length=20,
    width_segments=128,
